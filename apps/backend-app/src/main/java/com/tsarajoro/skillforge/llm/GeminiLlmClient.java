...
/**
 * Implementation Google Gemini via l endpoint OpenAI-compatible de Google.
 * Activee quand skillforge.llm.provider=gemini.
 *
 * <p>Free tier stable 2026 : Flash-Lite 15 req/min et 1000 req/jour,
 * Flash 10 req/min et 250 req/jour. Bien plus genereux que Groq et
 * bien plus rapide qu Ollama local.
 * Doc : https://ai.google.dev/gemini-api/docs/openai
 */
@Component
@ConditionalOnProperty(name = "skillforge.llm.provider", havingValue = "gemini")
public class GeminiLlmClient implements LlmClient {
    ...
    public GeminiLlmClient(
            @Value("${skillforge.llm.gemini.api-key}") String apiKey,
            @Value("${skillforge.llm.gemini.base-url}") String baseUrl,
            @Value("${skillforge.llm.gemini.model}") String model) {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException(
                    "GEMINI_API_KEY is missing while skillforge.llm.provider=gemini");
        }
        this.model = model;
    ...
    public CvExtractionResult extractSkillsFromCv(String cvText, String profileCode) {
        ...
        try {
            ...
            throw new LlmCallException("gemini chat/completions failed: " + e.getMessage(), e);
        }
    }
    ...
    public QuestionGenerationResult generateQuestions(GenerationRequest request) {
        ...
        try {
            ...
            throw new LlmCallException("gemini chat/completions (generation) failed: " + e.getMessage(), e);
        }
    }
    ...
    public CasGradingResult gradeCasPratique(String scenario, java.util.List<String> expectedPoints, String candidateAnswer) {
        ...
        try {
            ...
            throw new LlmCallException("gemini chat/completions (grading) failed: " + e.getMessage(), e);
        }
    }
    ...
    public ReportGenerationResult generateReport(ReportGenerationResult.Input input) {
        ...
        try {
            ...
            throw new LlmCallException("gemini chat/completions (report) failed: " + e.getMessage(), e);
        }
    }
