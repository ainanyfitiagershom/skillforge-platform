...
/**
 * Implementation Groq (API compatible OpenAI, mais free tier genereux).
 * Activee quand skillforge.llm.provider=groq.
 *
 * <p>Solution de repli apres le retrait annonce de GitHub Models fin 2026.
 * Groq propose Llama 3.3 70B et Mixtral gratuitement, 30 req/min.
 * Doc : https://console.groq.com/docs
 */
@Component
@ConditionalOnProperty(name = "skillforge.llm.provider", havingValue = "groq")
public class GroqLlmClient implements LlmClient {

    private static final Logger log = LoggerFactory.getLogger(GroqLlmClient.class);
    private static final String BASE_URL = "https://api.groq.com/openai/v1";
    // Groq est gratuit sur son free tier -> cost estime a 0.
    ...
    public GroqLlmClient(
            @Value("${skillforge.llm.groq.api-key}") String apiKey,
            @Value("${skillforge.llm.groq.model}") String model) {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException(
                    "GROQ_API_KEY is missing while skillforge.llm.provider=groq");
        }
        this.model = model;
    ...
    public CvExtractionResult extractSkillsFromCv(String cvText, String profileCode) {
        ...
        try {
            ...
            throw new LlmCallException("groq chat/completions failed: " + e.getMessage(), e);
        }
    }
    ...
    public QuestionGenerationResult generateQuestions(GenerationRequest request) {
        ...
        try {
            ...
            throw new LlmCallException("groq chat/completions (generation) failed: " + e.getMessage(), e);
        }
    }
    ...
    public CasGradingResult gradeCasPratique(String scenario, java.util.List<String> expectedPoints, String candidateAnswer) {
        ...
        try {
            ...
            throw new LlmCallException("groq chat/completions (grading) failed: " + e.getMessage(), e);
        }
    }
    ...
    public ReportGenerationResult generateReport(ReportGenerationResult.Input input) {
        ...
        try {
            ...
            throw new LlmCallException("groq chat/completions (report) failed: " + e.getMessage(), e);
        }
    }
